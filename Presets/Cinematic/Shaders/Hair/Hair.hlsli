...
namespace Hair
{
	...
	// TN Character Rebuild v0.6 - standalone Hair Specular shader addon
	// Preset: Cinematic. These constants replace the custom C++ constant-buffer
	// fields from v0.5 so the addon works with the stock CommunityShaders.dll.
	static const bool TN_ADDON_ENABLED = true;
	static const float TN_ADDON_INTENSITY = 1.00;
	static const float TN_ADDON_SEPARATION = 1.00;
	static const float TN_ADDON_FINE_STRANDS = 1.00;
	static const float TN_ADDON_CAVITY_DEPTH = 1.00;
	static const float TN_ADDON_HIGHLIGHT_REDUCTION = 1.00;
	...
	float TNMedievalStrandVariation(float2 uv)
	{
		...
		float fineWeight = 0.16 * clamp(TN_ADDON_FINE_STRANDS, 0.0, 1.5);
	...
	float TNAdaptiveStrandCavity(float2 uv, float3 baseColor)
	{
		...
		if (!TN_ADDON_ENABLED || TN_ADDON_INTENSITY <= 0.0)
			return 1.0;

		float intensity = clamp(TN_ADDON_INTENSITY, 0.0, 1.5);
		float separation = clamp(TN_ADDON_SEPARATION, 0.0, 1.5);
		...
		float cavityAmount = saturate(TN_ADDON_CAVITY_DEPTH * intensity);
	...
	float3 Saturation(float3 color, float saturation)
	{
		...
		float tnHighlight = TN_ADDON_ENABLED ?
			clamp(TN_ADDON_HIGHLIGHT_REDUCTION * TN_ADDON_INTENSITY, 0.0, 2.25) : 0.0;
