...
bool Streamline::GetDLSSDynamicResolutionBounds(uint32_t outputWidth, uint32_t outputHeight, DLSSDynamicResolutionBounds& outBounds)
{
	outBounds = {};
	if (!featureDLSS || !slDLSSGetOptimalSettings || outputWidth == 0 || outputHeight == 0)
		return false;
	...
	if (result != sl::Result::eOk) {
		logger::warn("[Streamline] slDLSSGetOptimalSettings failed for {}x{}: {}",
			outputWidth, outputHeight, magic_enum::enum_name(result));
		return false;
	}

	outBounds.optimalRenderWidth = settings.optimalRenderWidth;
	outBounds.optimalRenderHeight = settings.optimalRenderHeight;
	outBounds.renderWidthMin = settings.renderWidthMin;
	outBounds.renderHeightMin = settings.renderHeightMin;
	outBounds.renderWidthMax = settings.renderWidthMax;
	outBounds.renderHeightMax = settings.renderHeightMax;

	const auto scaleFromDimensions = [&](uint32_t width, uint32_t height) {
		const float sx = static_cast<float>(width) / static_cast<float>(outputWidth);
		const float sy = static_cast<float>(height) / static_cast<float>(outputHeight);
		return std::clamp(std::min(sx, sy), 0.01f, 1.0f);
	};

	if (settings.optimalRenderWidth > 0 && settings.optimalRenderHeight > 0)
		outBounds.optimalScale = scaleFromDimensions(settings.optimalRenderWidth, settings.optimalRenderHeight);

	const bool hasDynamicRange =
		settings.renderWidthMin > 0 && settings.renderHeightMin > 0 &&
		settings.renderWidthMax > 0 && settings.renderHeightMax > 0 &&
		settings.renderWidthMax >= settings.renderWidthMin &&
		settings.renderHeightMax >= settings.renderHeightMin;

	if (hasDynamicRange) {
		// A scalar scale must satisfy both axes. The minimum therefore uses the
		// larger axis ratio, while the maximum uses the smaller axis ratio.
		const float minX = static_cast<float>(settings.renderWidthMin) / static_cast<float>(outputWidth);
		const float minY = static_cast<float>(settings.renderHeightMin) / static_cast<float>(outputHeight);
		const float maxX = static_cast<float>(settings.renderWidthMax) / static_cast<float>(outputWidth);
		const float maxY = static_cast<float>(settings.renderHeightMax) / static_cast<float>(outputHeight);
		outBounds.minScale = std::clamp(std::max(minX, minY), 0.01f, 1.0f);
		outBounds.maxScale = std::clamp(std::min(maxX, maxY), outBounds.minScale, 1.0f);
		outBounds.dynamicSupported = outBounds.maxScale - outBounds.minScale > 0.005f;
		outBounds.valid = true;
	} else if (settings.optimalRenderWidth > 0 && settings.optimalRenderHeight > 0) {
		// If the plugin does not expose a dynamic range, lock AD80 to NVIDIA's
		// optimal source size rather than inventing unsupported resolutions.
		outBounds.minScale = outBounds.optimalScale;
		outBounds.maxScale = outBounds.optimalScale;
		outBounds.renderWidthMin = outBounds.renderWidthMax = settings.optimalRenderWidth;
		outBounds.renderHeightMin = outBounds.renderHeightMax = settings.optimalRenderHeight;
		outBounds.dynamicSupported = false;
		outBounds.valid = true;
	}
