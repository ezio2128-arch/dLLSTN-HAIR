...
public:
	...
	struct DLSSDynamicResolutionBounds
	{
		bool valid = false;
	...
	/** Query the DLSS plugin for the source-resolution range supported by the active quality mode. */
	bool GetDLSSDynamicResolutionBounds(uint32_t outputWidth, uint32_t outputHeight, DLSSDynamicResolutionBounds& outBounds);
