...
public:
	...
	// TN Adaptive 80 v0.3: a quantized render-size transition can request one
	// frame-generation history reset. It is consumed once in Present().
	static inline std::atomic<bool> adaptiveScaleResetRequested = false;
	...
	void RequestFrameGenerationHistoryReset() { adaptiveScaleResetRequested.store(true, std::memory_order_seq_cst); }
