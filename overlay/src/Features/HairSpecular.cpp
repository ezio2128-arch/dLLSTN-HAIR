...
static_assert(sizeof(HairSpecular::Settings) % 16 == 0, "HairSpecular settings must stay 16-byte aligned with SharedData.hlsli");
...
void HairSpecular::DrawSettings()
{
	...
	ImGui::Spacing();
	ImGui::SeparatorText(T(TKEY("tn_medieval_hair"), "TN Medieval Hair"));
	ImGui::Checkbox(T(TKEY("tn_enabled"), "Enable TN Medieval Hair"), (bool*)&settings.TNEnabled);
	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::TextUnformatted(T(TKEY("tn_enabled_tooltip"), "Toggle the True North strand separation and cinematic material reconstruction without disabling Hair Specular."));
	}
	...
	if (settings.TNEnabled) {
		...
		ImGui::SliderFloat(T(TKEY("tn_separation"), "Strand Separation"), &settings.TNSeparation, 0.0f, 1.5f, "%.2f");
		ImGui::SliderFloat(T(TKEY("tn_fine_strands"), "Fine Strands"), &settings.TNFineStrands, 0.0f, 1.5f, "%.2f");
		ImGui::SliderFloat(T(TKEY("tn_cavity_depth"), "Cavity Depth"), &settings.TNCavityDepth, 0.0f, 1.5f, "%.2f");
		ImGui::SliderFloat(T(TKEY("tn_highlight_reduction"), "Highlight Reduction"), &settings.TNHighlightReduction, 0.0f, 1.5f, "%.2f");
	}
}
